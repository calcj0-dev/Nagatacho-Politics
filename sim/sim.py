...
class SimGame:
    ...
    def exec_ability(self, effect_id, side):
        ...
        fx = {
            ...
            "kono_1":      lambda: (G.change_approval(self_, 3), self_["shields"].append("block_approval_down")),
            "kono_2":      lambda: (G.change_approval(self_, 8), self_["deck"] and self_["hand"].append(self_["deck"].pop(0))),
            "suga_1":      lambda: G.change_funds(self_, 3),
            "suga_2":      lambda: G.change_approval(self_, 12 + (8 if len(self_["field"]) == 1 else 0)),
            "tamaki_1":    lambda: self_["field"] and G.change_approval(opp, -(len(self_["field"]) * 4)),
            "tamaki_2":    lambda: G.change_funds(self_, 4),
            ...
            "nakatsuka_2": lambda: G.change_funds(self_, 5),
